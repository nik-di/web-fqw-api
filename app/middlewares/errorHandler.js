...
module.exports.errorHandler = (err, req, res, next) => { // eslint-disable-line
  ...
  if (isCelebrate(err)) {
    res.status(400).send({ status: 400, message: CelebrateError(err.joi).message });
    return;
  }

  if (err instanceof mongoose.Error.ValidationError) {
    const errors = Object.values(err.errors).map((error) => error.message);
    res.status(400).send({ status: statusCode, message: { ...errors } });
    return;
  }
  ...
  res
    .status(statusCode)
    .send({ status: statusCode, message: resultMessage });
};
